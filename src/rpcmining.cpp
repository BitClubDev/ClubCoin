...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest",    (uint64_t)GetDynamicBlockHeightPoSAward(pindexBest->nHeight)));
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain) {
		...
		int64_t stake = pwalletMain->GetStake();
		int64_t unconfirmedBalance = pwalletMain->GetUnconfirmedBalance();
		int64_t immatureBalance = pwalletMain->GetImmatureBalance();
        wallet.push_back(Pair("balance", balance));
		wallet.push_back(Pair("stake", stake));
		wallet.push_back(Pair("unconfirmed", unconfirmedBalance));
		wallet.push_back(Pair("immature", immatureBalance));
		wallet.push_back(Pair("total", balance + stake + unconfirmedBalance + immatureBalance));
		obj.push_back(Pair("wallet", wallet));
    }
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ClubCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ClubCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ClubCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ClubCoin is downloading blocks...");
